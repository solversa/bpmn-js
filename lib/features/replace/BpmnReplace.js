...
function BpmnReplace(bpmnFactory, moddle, popupMenu, replace, selection) {
  ...
  function getToggleOptions(element) {
    ...
    var isAdHoc = is(element, 'bpmn:AdHocSubProcess'),
        isSequential,
        isLoop,
        isParallel,
    ...
    return [
      {
        ...
        action: function(event, target){
          if (target.active) {
            popupMenu.update('toggle-sequential-mi', { className: 'icon-end-event-terminate'});
            popupMenu.update(target, { active: false });
          } else {
            popupMenu.update('toggle-sequential-mi', { className: 'icon-intermediate-event-none'});
            popupMenu.update(target, { active: true });
          }
        }
      },
      {
      ...
      },
      {
        ...
        className: 'icon-compensation-marker',
        active: isForCompensation
      },
      {
        id: 'toggle-adhoc',
  ...
  this.openChooser = function(position, element) {
    ...
    var headerEntries = this.getToggleOptions(element);
